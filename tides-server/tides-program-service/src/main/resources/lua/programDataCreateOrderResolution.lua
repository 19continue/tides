-- 类型 1 用户选座位 2自动匹配座位
local order_seat_type = tonumber(ARGV[1])
-- 没有售卖的座位key
local placeholder_seat_no_sold_hash_key = KEYS[1]
-- 锁定的座位key
local placeholder_seat_lock_hash_key = KEYS[2]
-- 节目id
local program_id = ARGV[5]
-- 记录的key 对应的真正数据是 tides_program_record_%s
local record_hash_key = KEYS[3]
-- 记录标识  对应的真正数据是 reduce_993369070199742464_992950774744588290
local identifier_id = ARGV[6]
-- 记录类型 对应的真正数据是 reduce
local record_type = ARGV[7]
-- 要购买的票档 包括票档id和票档数量
local ticket_count_list = cjson.decode(ARGV[2])
-- 购票人id集合 对应的数据是购票人id集合
local ticket_user_id_list = cjson.decode(ARGV[4])
local function redis_id(value)
    if not value then
        return nil
    end
...
local function get_ticket_user_id(index)
    local ticket_user_id = ticket_user_id_list[index]
    if not ticket_user_id then
        return nil
    end
    return redis_id(ticket_user_id)
end
-- 过滤后符合条件可以购买的座位集合
...
-- 不选座演出只需要占用同票档库存，不要求座位连续。
local function take_available_seats(all_seats, seat_count)
    local selected_seats = {}
    if (#all_seats < seat_count) then
    ...
    for i = 1, seat_count do
        table.insert(selected_seats, all_seats[i])
    end
    return selected_seats
end

-- 入参座位存在
if (order_seat_type == 1) then
    ...
    local seat_index = 0
    for index, seatData in ipairs(seat_data_list) do
        -- 没有售卖的座位key
        ...
        for index2,seat_dto in ipairs(seat_dto_list) do
            ...
            if (tonumber(seat_dto.ticketCategoryId) ~= tonumber(seat_vo.ticketCategoryId) or
                    tonumber(seat_dto.rowCode) ~= tonumber(seat_vo.rowCode) or
                    tonumber(seat_dto.colCode) ~= tonumber(seat_vo.colCode)) then
                return string.format('{"%s": %d}', 'code', 40031)
            end
            if (tonumber(seat_dto_price) ~= tonumber(seat_vo.price)) then
            ...
            local ticket_user_id = get_ticket_user_id(seat_index)
            for index3, ticket_category_record in pairs(ticket_category_record_list) do
                if ticket_category_record.ticketCategoryId == seat_vo.ticketCategoryId then
                    ...
                    -- 绑定上购票人id
                    seat_record.ticketUserId = ticket_user_id
                    seat_vo.ticketUserId = ticket_user_id
...
if (order_seat_type == 2) then
    ...
    for index,ticket_count in ipairs(ticket_count_list) do
        ...
        -- 自动分配没有匹配到连续座位时，降级为任意可售座位。
        if (#purchase_seat_list < count) then
            purchase_seat_list = take_available_seats(filter_seat_vo_no_sold_list,count)
        end
        ...
        for index2,purchase_seat in ipairs(purchase_seat_list) do
            purchase_seat.id = redis_id(purchase_seat.id)
            local ticket_user_id = get_ticket_user_id(index2)
            -- 先构建好座位记录
            if not ticket_category_record.seatRecordList then
                ticket_category_record.seatRecordList = {}
            end
            ...
            seat_record.ticketUserId = ticket_user_id
            -- 绑定上购票人id
            purchase_seat.ticketUserId = ticket_user_id
            table.insert(ticket_category_record.seatRecordList,seat_record)
        end
