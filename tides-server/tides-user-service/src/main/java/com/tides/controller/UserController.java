...
import com.tides.service.UserService;
...
public class UserController {
    ...
    @Operation(summary  = "演示压测批量准备用户")
    @PostMapping(value = "/simulator/prepare")
    public ApiResponse<SimulatorUserPrepareVo> simulatorPrepare(
            @RequestHeader(value = "X-Tides-Simulator-Token", required = false) String simulatorToken,
            @RequestBody SimulatorUserPrepareDto simulatorUserPrepareDto) {
        return ApiResponse.ok(userService.prepareSimulatorUsers(simulatorToken, simulatorUserPrepareDto));
    }
