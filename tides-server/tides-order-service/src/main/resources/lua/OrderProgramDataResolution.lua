...
local operate_order_status = tonumber(ARGV[5])
-- 节目id
local program_id = ARGV[6]
-- 记录的key
local record_hash_key = KEYS[1]
-- 记录标识
local identifier_id = ARGV[7]
-- 记录类型
local record_type = ARGV[8]
-- 解除锁定的座位id列表
...
local lock_status = 2
local source_status = tonumber(ARGV[9]) or lock_status
-- 座位id和购票人id的映射
...
for index, add_seat_data_json_object in pairs(add_seat_data_json_array) do
    ...
    for i = 2, #seat_data_list, 2 do
        ...
        seat_record.beforeStatus = source_status
...
-- 如果是将订单取消
if (operate_order_status == 2 or operate_order_status == 4) then
    -- 恢复库存
