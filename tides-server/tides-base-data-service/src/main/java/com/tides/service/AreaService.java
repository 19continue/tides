...
import com.tides.entity.Area;
import com.tides.enums.AreaType;
import com.tides.enums.BusinessStatus;
import com.tides.mapper.AreaMapper;
...
public class AreaService extends ServiceImpl<AreaMapper, Area> {
    ...
    public List<AreaVo> manageList(){
        final LambdaQueryWrapper<Area> lambdaQueryWrapper = Wrappers.lambdaQuery(Area.class)
                .eq(Area::getStatus, BusinessStatus.YES.getCode())
                .in(Area::getType, AreaType.PROVINCE.getCode(), AreaType.MUNICIPALITIES.getCode(),
                        AreaType.PREFECTURE.getCode())
                .orderByAsc(Area::getType)
                .orderByAsc(Area::getParentId)
    ...
    public List<AreaVo> hot() {
        final LambdaQueryWrapper<Area> lambdaQueryWrapper = Wrappers.lambdaQuery(Area.class)
                .in(Area::getName, "全国","北京","上海","深圳","广州","杭州","天津","重庆","成都","嘉兴","中国香港");
        List<Area> areas = areaMapper.selectList(lambdaQueryWrapper);
        return BeanUtil.copyToList(areas,AreaVo.class);
    }
}
