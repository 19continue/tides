...
import { orderPageQueryApi } from '#/api/order';
...
const [Grid, gridApi] = useVbenVxeGrid({
    ...
    proxyConfig: {
      ...
      ajax: {
        query: async ({ page }, formValues) => {
          ...
          const res: any = await orderPageQueryApi(query);
  ...
  } as VxeTableGridOptions<OrderMangeApi.OrderPageQueryResult>,
});
...
<template>
  <Page auto-content-height>
    ...
    <Grid :table-title="$t('tides.index.orderListTitle')">
      ...
      <template #ticketExpand="{ row }">
        <div class="p-2 bg-gray-50 rounded">
          <table class="table-fixed w-full text-left text-sm">
            <thead>
              <tr>
                <th class="py-1 px-2 w-[20%]">购票人订单id</th>
                <th class="py-1 px-2 w-[20%]">购票人id</th>
                <th class="py-1 px-2 w-[15%]">座位信息</th>
                <th class="py-1 px-2 w-[10%]">价格</th>
                <th class="py-1 px-2 w-[10%]">状态</th>
                <th class="py-1 px-2 w-[12%]">生成时间</th>
                <th class="py-1 px-2 w-[12%]">支付时间</th>
                <th class="py-1 px-2 w-[12%]">取消时间</th>
              </tr>
            </thead>
            <tbody>
              <tr v-for="(d, idx) in (row.orderTicketUserManageVoList || [])" :key="idx">
                <td class="py-1 px-2 truncate">{{ d.id }}</td>
                <td class="py-1 px-2 truncate">{{ d.ticketUserId }}</td>
                <td class="py-1 px-2 truncate">{{ d.seatInfo }}</td>
                <td class="py-1 px-2">{{ d.orderPrice }}</td>
                <td class="py-1 px-2">{{ d.orderStatusName }}</td>
                <td class="py-1 px-2">{{ d.createOrderTime }}</td>
                <td class="py-1 px-2">{{ d.payOrderTime }}</td>
                <td class="py-1 px-2">{{ d.cancelOrderTime }}</td>
              </tr>
            </tbody>
          </table>
        </div>
