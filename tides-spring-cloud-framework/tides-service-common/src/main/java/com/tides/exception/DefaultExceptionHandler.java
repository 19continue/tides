...
public class DefaultExceptionHandler {
    ...
    * 业务异常
    * */
    @ExceptionHandler(value = TidesFrameException.class)
    public ApiResponse<String> toolkitExceptionHandler(HttpServletRequest request, TidesFrameException tidesFrameException) {
        log.error("业务异常 错误信息 : {} method : {} url : {} query : {} ", tidesFrameException.getMessage(), request.getMethod(), getRequestUrl(request), getRequestQuery(request), tidesFrameException);
        return ApiResponse.error(tidesFrameException.getCode(), tidesFrameException.getMessage());
    }
    /**
     * 参数验证异常
     */
